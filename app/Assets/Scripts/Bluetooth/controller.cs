...
public class controller : MonoBehaviour
{
    ...
    public bool _readFound2 = false; // currently a redundant variable
    ...
    private bool _ppgBodyLocation_Notification = false;
    private bool _SkinConductance_Notification = false;
    private bool _gsrBodyLocation_Notification = false;
    private bool _FirmwareRevision_Notification = false;
    ...
    public bool _storeReadFakeData = false;
    ...
    public void disconnectBluetooth(bool _calledThroughForgetDevice)
    {
        ...
        BluetoothLEHardwareInterface.DisconnectPeripheral(_connectedAddress, connectedDeviceAddress =>
        {
            ...
            _ppgBodyLocation_Notification = false;
            _SkinConductance_Notification = false;
            _gsrBodyLocation_Notification = false;
            _FirmwareRevision_Notification = false;
            _ppgBodyCheck = false;
            _gsrBodyCheck = false;
    ...
    void uint8_tDecode(byte[] raw_data, string service, string characteristic)
    {
        ...
        else if (service == _gsrServiceUUID && characteristic == _bodySensorLocationGsrUUID)
        {
    ...
    void subscribeToCharacteristic(string serviceUUID, string characteristicUUID, Action<byte[], string, string> Decoder)
    {
        BluetoothLEHardwareInterface.SubscribeCharacteristicWithDeviceAddress(_connectedAddress, serviceUUID, characteristicUUID, (deviceAddress, notification) =>
        {
            ...
            if (serviceUUID == _ppgServiceUUID && characteristicUUID == _HRmeasurementUUID) { _HR_Notification = true; }
            if (serviceUUID == _ppgServiceUUID && characteristicUUID == _InterbeatIntervalUUID) { _IBI_Notification = true; }
            if (serviceUUID == _ppgServiceUUID && characteristicUUID == _Spo2MeasurementUUID) { _Spo2_Notification = true; }
            if (serviceUUID == _ppgServiceUUID && characteristicUUID == _bodySensorLocationUUID) { _ppgBodyLocation_Notification = true; }
            if (serviceUUID == _gsrServiceUUID && characteristicUUID == _skinConductanceLevelUUID) { _SkinConductance_Notification = true; }
            if (serviceUUID == _gsrServiceUUID && characteristicUUID == _bodySensorLocationGsrUUID) { _gsrBodyLocation_Notification = true; }
            if (serviceUUID == _DeviceInfoUUID && characteristicUUID == _FirmwareRevisionUUID) { _FirmwareRevision_Notification = true; }
    ...
    void Update()
    {
        ...
        if (_readFound == true)
        {
            ...
            if (count == 15)
            {
                if (_ppgBodyLocation_Notification != true) { subscribeToCharacteristic(_ppgServiceUUID, _bodySensorLocationUUID, uint8_tDecode); }
            }
            ...
            if (count == 25)
            {
                if (_FirmwareRevision_Notification != true) { subscribeToCharacteristic(_DeviceInfoUUID, _FirmwareRevisionUUID, UTF8Decode); }
            }

            if (count == 30)
            {
                count = 0;
                if (_HR_Notification == true && _IBI_Notification == true && _Spo2_Notification == true && _SkinConductance_Notification == true && _ppgBodyLocation_Notification == true && _FirmwareRevision_Notification == true)
                {
                    _readFound = false;
